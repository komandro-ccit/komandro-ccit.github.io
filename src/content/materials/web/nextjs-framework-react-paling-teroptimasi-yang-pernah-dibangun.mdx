...
graph TD
    ...
    A1["&#9679; Turbopack (Rust-based)<br/>&#9679; Static Generation<br/>&#9679; Tree Shaking<br/>&#9679; Code Splitting"] -.-> A
    B1["&#9679; SSR + Streaming<br/>&#9679; RSC Execution<br/>&#9679; Edge Runtime<br/>&#9679; Cache Components"] -.-> B
    C1["&#9679; CDN Edge Caching<br/>&#9679; Compressed Assets<br/>&#9679; Incremental Prefetching<br/>&#9679; Optimized Images"] -.-> C
    D1["&#9679; Automatic Code Split<br/>&#9679; Lazy Loading<br/>&#9679; React Compiler (auto-memo)<br/>&#9679; Font Self-Hosting"] -.-> D
    E1["&#9679; Convention over Config<br/>&#9679; File-based Routing<br/>&#9679; Fast Refresh<br/>&#9679; TypeScript First-Class"] -.-> E
...
flowchart LR
    ...
    subgraph "ISR: Runtime Revalidation"
        ...
        G -->|Yes| H[🚀 Serve cached HTML<br/>instant, CDN]
...
graph TD
    subgraph "Server (Node.js / Edge)"
        SC1["📄 Server Component<br/>Layout (nav, footer)"]
        SC2["📄 Server Component<br/>BlogPost (fetch data)"]
        SC3["📄 Server Component<br/>ProductList (DB query)"]
        SC4["📄 Server Component<br/>MarkdownRenderer (heavy lib)"]
    end

    subgraph "Client (Browser)"
        CC1["🖱️ Client Component<br/>LikeButton (useState)"]
        CC2["🖱️ Client Component<br/>SearchInput (onChange)"]
        CC3["🖱️ Client Component<br/>CartDrawer (useEffect)"]
    end
...
```mermaid
graph TD
    Root["🏠 Root Layout<br/>app/layout.tsx<br/>&lt;html&gt; + providers + global nav"]

    Root --> Blog["📝 Blog Layout<br/>app/blog/layout.tsx<br/>Sidebar blog + categories"]
    Root --> Dashboard["📊 Dashboard Layout<br/>app/dashboard/layout.tsx<br/>Auth check + sidebar admin"]

    Blog --> Post1["📄 /blog/post-1<br/>app/blog/post-1/page.tsx<br/>Konten artikel"]
    Blog --> Post2["📄 /blog/post-2<br/>app/blog/post-2/page.tsx<br/>Konten artikel"]

    Dashboard --> DashHome["📊 /dashboard<br/>app/dashboard/page.tsx"]
    Dashboard --> DashSettings["⚙️ /dashboard/settings<br/>app/dashboard/settings/page.tsx"]
...
graph TD
    ...
    subgraph Layer1["Layer 1: Request Memoization"]
        RM["React <code>cache()</code><br/>Deduplikasi fetch dalam 1 request<br/>Lifetime: per-render pass"]
    end

    Layer1 --> Layer2

    subgraph Layer2["Layer 2: Data Cache"]
        DC["<code>fetch()</code> cache / <code>unstable_cache</code><br/>Cache hasil fetch/data query<br/>Lifetime: persistent (cross-request)"]
    end

    Layer2 --> Layer3

    subgraph Layer3["Layer 3: Full Route Cache"]
        RC["HTML + RSC Payload cache<br/>Cache seluruh output halaman<br/>Lifetime: persistent (di server)"]
    end

    Layer3 --> Layer4

    subgraph Layer4["Layer 4: Router Cache"]
        RtC["Client-side in-memory cache<br/>Cache RSC payload di browser<br/>Lifetime: session / staleTimes"]
    end
...
sequenceDiagram
    ...
    Note over Page: Promise.all resolves<br/>Total time ≈ slowest request (not sum!)
```
...
graph TD
    ...
    EdgeJP["🇯🇵 Edge Node Tokyo<br/>< 30ms latency"]
    EdgeEU["🇬🇧 Edge Node London<br/>< 20ms latency"]
    EdgeBR["🇧🇷 Edge Node Sao Paulo<br/>< 25ms latency"]

    Origin["🖥️ Origin Server<br/>US East<br/>150-300ms latency"]
